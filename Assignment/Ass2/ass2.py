...
m.setObjective(quicksum(C[q] * I[q, p] for p in P for q in Q)
                + quicksum(StorePrice * S[q, p] for p in P for q in Q)
...
for q in Q:
    for p in P:
        m.addConstr(I[q, p] <=  Y[q, p] * ShipCap) #com 4
        if q >= 1:
...
for q in Q:
    m.addConstr(quicksum(I[q, p] * Y[q, p] for p in P) <= ShipCap * X[q] ) # com4
...
print("\nY values")
for q in Q:
    print([round(Y[q, p].x) for p in P])
