...
Z2 = {(q, h): m.addVar(vtype=GRB.BINARY) for h in H2 for q in Q}
...
for q in Q:
    for h in H:
        m.addConstr(X[q] - X2[q, h] >= 0)

for q in Q:
    for h in H2:
        m.addConstr(Z[q] - Z2[q, h] >= 0)
        
for q in Q:
    m.addConstr(quicksum(X2[q, h] for h in H)  + quicksum(Z2[q, h] for h in H2) 
                >= quicksum(I[q, p] for p in P)/1000)
    
    m.addConstr(5 + Z[q]*3 - quicksum(X2[q, h] for h in H) - quicksum(Z2[q, h] for h in H2) 
                >= quicksum(I2[q, p] for p in P)/1000)
    #com 8
for q in Q:
    if q == 7:
        break
    m.addConstr(Z[q] + Z[q+1] <= 1)
...
print("\nX2 value")
for q in Q:
    print([X2[q, h].x for h in H])

print("\nZ2 value")
for q in Q:
    print([Z2[q, h].x for h in H2])
