...
VAR_OF_FOOD = {}
for f in F:
    VAR_OF_FOOD[f] = m.addVar(vtype=GRB.INTEGER)

m.setObjective(quicksum(C[f] * VAR_OF_FOOD[f] for f in F), GRB.MINIMIZE)

for n in N:
    m.addConstr(quicksum(NV[f][n] * VAR_OF_FOOD[f] for f in F) <= DMAX[n])
    # for f in F:
    #   if X[f].x>0:
    #       print("EAT",X[f].x"of",Food[f])

    # for n in N:
    # if sum(NV[f][n]*x[f].x for f in F)> DMAX[n]:
    #    print(Nutrients[x],DMAX[n],sum(NV[f][n]*X[f].x for f in F))

    m.addConstr(quicksum(NV[f][n] * VAR_OF_FOOD[f] for f in F) >= DMIN[n])
m.optimize()

for f in F:
    if VAR_OF_FOOD[f].x > 0:
        print("Eat", VAR_OF_FOOD[f].x, Food[f])

print("The Cost of Subsistence is ", m.objVal)
